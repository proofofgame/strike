;; Raid Pass | Strike | Raid Pass NFT | v.1.0.0
;; skullco.in
...
;; Define NFT token
(define-non-fungible-token raid-pass uint)
...
(define-constant ERR-METADATA-FROZEN (err u304))
(define-constant ERR-MINT-ALREADY-SET (err u305))
(define-constant ERR-LISTING (err u306))
(define-constant ERR-ALREADY-MINTED (err u307))
...
(define-public (mint (new-owner principal))
    (let ((next-id (+ u1 (var-get last-id))))
      ...
      (asserts! (is-eq (get-balance new-owner) u0) ERR-ALREADY-MINTED)
      (match (nft-mint? raid-pass next-id new-owner)
        success
        (let
...
(define-private (trnsfr (id uint) (sender principal) (recipient principal))
  (match (nft-transfer? raid-pass id sender recipient)
        success
          (let
...
(define-private (is-sender-owner (id uint))
  (let ((owner (unwrap! (nft-get-owner? raid-pass id) false)))
...
(define-public (buy-in-ustx (id uint) (comm <commission-trait>))
  (let ((owner (unwrap! (nft-get-owner? raid-pass id) ERR-NOT-FOUND))
